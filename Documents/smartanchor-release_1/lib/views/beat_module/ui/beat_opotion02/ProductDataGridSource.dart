...
class ProductDataGridSource extends DataGridSource {
  ...
  ProductDataGridSource(String sampleType, {required this.productDataCount, columns}) {
    ...
    dataGridRows = products.map<DataGridRow>((Product product) {
      if (sampleType == 'Stacked Header') {
        return DataGridRow(cells: <DataGridCell<dynamic>>[
          ...
          DataGridCell<DateTime>(columnName: 'orderDate', value: product.orderDate),
    ...
    if (_sampleName == 'Stacked Header') {
      return DataGridRowAdapter(cells: <Widget>[
        ...
        Container(
          ...
          padding: const EdgeInsets.all(8),
          child: Text(
            NumberFormat.currency(locale: 'en_US', symbol: r'$').format(row.getCells()[7].value),
            overflow: TextOverflow.ellipsis,
          ),
        ),
      ]);
      ...
      return DataGridRowAdapter(
        ...
        final DataGridCell cell = row.getCells().firstWhere((cell) => cell.columnName == e.columnName);

        return Container(
          alignment: (cell.columnName == 'name' || cell.columnName == 'product' || cell.columnName == 'city') ? Alignment.centerLeft : Alignment.centerRight,
  ...
  List<Product> getProducts(int count) {
    ...
    for (int i = 0; i < count; i++) {
      productData.add(
        Product(
            i + 1000,
            _productId[i < _productId.length ? i : _random.nextInt(_productId.length - 1)],
            _product[i < _product.length ? i : _random.nextInt(_product.length - 1)],
  ...
  int compare(DataGridRow? a, DataGridRow? b, SortColumnDetails sortColumn) {
    if (_sampleName == 'Custom Sorting' && sortColumn.name == 'name') {
      final String? value1 = a?.getCells().firstWhereOrNull((dynamic element) => element.columnName == sortColumn.name)?.value?.toString();
      final String? value2 = b?.getCells().firstWhereOrNull((dynamic element) => element.columnName == sortColumn.name)?.value?.toString();
